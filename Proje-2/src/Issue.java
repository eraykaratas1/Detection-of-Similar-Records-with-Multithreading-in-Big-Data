...
public class Issue implements Runnable{
   ...
   private ArrayList<String> issue_data;
    ...
    public Issue(ArrayList<String> issue,int thread_value,ArrayList<String> issue_data) {
        this.issue =issue;
    ...
    public void run() {
        ...
        for (int i=1;i<issue.size();i++) {  // diger kayitlarda aranacak kayit
            
            String x=issue.get(i);    // aranacak kayit
            ...
            for (int j=thread_value*10000+1-10000;j<thread_value*10000; j++) {  // diger kayitlar
              ...
              // 1.kayıtta 2.kayıttan basla,2.kayıtta 3.kayıttan basla,3.kayıtta 4.kayıttan basla
               ...
               String y=issue.get(j);   // kiyaslanacak kayit
         ...
         if(str.length>=str2.length){
           ...
           similar=(l1/str.length)*100;     //str nin boyutu str 2 den daha buyukse payda str olur,kayitlar arasindaki benzerlik orani 
            ...
            issue_data.add(d1);
            issue_data.add(x);  // aranacak kayıt
            
            issue_data.add(d2);
            issue_data.add(y);  // kıyaslanacak kayıt
            
            issue_data.add(sim1);  // benzerlik oranı
            ...
            String sim1=String.valueOf(similar);
             ...
             issue_data.add(d1);  // aranacak kayisin sirasi
             issue_data.add(x);  // aranacak kayıt
            
             issue_data.add(d2); // kiyaslanacak kayidin sirasi
             issue_data.add(y);  // kıyaslanacak kayıt
            
             issue_data.add(sim1);  // benzerlik oranı
         }

         System.out.println("Issue Sutunundaki "+i+".ci kayit ile "+j+".ci kayit arasindaki benzerlik orani:%"+similar); 
